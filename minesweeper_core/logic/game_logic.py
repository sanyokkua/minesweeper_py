...
class GameLogic:
    ...
    def __init__(self, game_config: Configuration) -> None:
        ...
        self._init_field()
        log.debug('Game inited with: field: %s, first_time: %s, finished: %s, win: %s',
    ...
    def flag_cell(self, row: int, column: int) -> None:
        ...
        log.debug('flag_cell, (%d, %d) flag_before -> %s', row, column, cell.has_flag)
        if cell.has_flag:
            cell.has_flag = False
            self._flags_number += 1
        elif not cell.has_flag and self._flags_number > 0:
            cell.has_flag = True
            self._flags_number -= 1
        log.debug('flag_cell, (%d, %d) flag_after -> %s', row, column, cell.has_flag)
    ...
    def _put_mines(self, current_coordinate: tuple[int, int]) -> None:
        ...
        while len(coordinates) < number_of_mines:
            ...
            if coord in coordinates or coord == current_coordinate:
                ...
                continue  # We do not want to have coordinates that already in the list or
        ...
        for cell_coordinate in coordinates:
            ...
            cell.has_mine = True
            log.debug('_put_mines, put mine to coordinate: %s, has_mine: %s', cell_coordinate,
                      cell.has_mine)
    ...
    def _finish_game(self, is_player_exploded: bool = False) -> None:
        ...
        if self._is_player_win:
            for cell in self.field.values():
                if cell.has_mine:
                    cell.has_flag = True
        log.debug('_finish_game. Result {_is_game_finished: %s, _is_player_win: %s}',
                  self._is_game_finished, self._is_player_win)

    def _open_this_and_neighbour_cells(self, coordinate: tuple[int, int]) -> None:
        ...
        if self.field[coordinate].neighbour_mines > 0:
            ...
            log.debug('_open_this_and_neighbour_cells, skip opening, because has neighbour mines')
            return
        queue: set[tuple[int, int]] = set()
        queue.add(coordinate)
        while len(queue) > 0:
            pop_coordinate: tuple[int, int] = queue.pop()
            log.debug('_open_this_and_neighbour_cells, processing pop_coordinate: %s', pop_coordinate)
            current_cell: Cell = self.field[pop_coordinate]
            if utils.is_alone_cell(current_cell):
                log.debug('_open_this_and_neighbour_cells, utils.is_alone_cell(current_cell)')
                current_cell.is_open = True
                neighbours = utils.get_neighbour_coordinates(pop_coordinate, self.rows, self.columns)
                for new_coordinate in neighbours:
                    queue.add(new_coordinate)
            elif utils.has_neighbour_mines(current_cell):
                log.debug('_open_this_and_neighbour_cells, utils.has_neighbour_mines(current_cell)')
                current_cell.is_open = True
    ...
    def _count_neighbour_mines_for_cell(self, cell: Cell) -> None:
        ...
        log.debug('_count_neighbour_mines_for_cell, cell: %s, mines_around: %d',
    ...
    def _get_neighbour_cells(self, row: int, column: int) -> dict[tuple[int, int], Cell]:
        ...
        neighbours = utils.get_neighbour_coordinates((row, column), self.rows, self.columns)
        ...
        log.debug('_get_neighbour_cells. for (%d, %d), number of neighbours: %d',
    ...
    def _process_current_game_state(self):
        ...
        log.debug('_process_current_game_state. num_mines: %d, num_cells: %d', expected_number_of_mines,
                  expected_number_of_cells)
        count_opened_cells: int = 0
        for cell in self.field.values():
            if cell.is_open:
                count_opened_cells += 1
        log.debug('_process_current_game_state. opened: %d', count_opened_cells)
        total_opened_cells_and_plus_mines: int = count_opened_cells + expected_number_of_mines
        log.debug('_process_current_game_state. total_opened_cells_and_plus_mines: %d',
                  total_opened_cells_and_plus_mines)
        if total_opened_cells_and_plus_mines == expected_number_of_cells:
            log.debug('_process_current_game_state. Game is Finished without flags')
